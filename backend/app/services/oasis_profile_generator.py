"""
OASIS Agent Profile
ZepgraphOASISsimulationplatformAgent Profile
...
class OasisAgentProfile:
    ...
    def to_reddit_format(self) -> Dict[str, Any]:
        ...
        profile = {
            ...
            "username": self.user_name, # OASIS username（none）
            "name": self.name,
            "bio": self.bio,
...
class OasisProfileGenerator:
    ...
    ZepgraphOASISsimulationAgent Profile
    ...
    def __init__(
        ...
        if self.zep_api_key:
            try:
                ...
                logger.warning(f"Zepinitializefailed: {e}")
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.debug(f"Zepedge {attempt + 1} failed: {str(e)[:80]},...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"Zepedge {max_retries} failed: {e}")
            return None
        ...
        def search_nodes():
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    last_exception = e
                    if attempt < max_retries - 1:
                        logger.debug(f"Zepnode {attempt + 1} failed: {str(e)[:80]},...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"Zepnode {max_retries} failed: {e}")
            return None
        ...
        try:
            ...
            logger.info(f"Zep: {entity_name}, get {len(results['facts'])}, {len(results['node_summaries'])} node")
            
        except concurrent.futures.TimeoutError:
            logger.warning(f"Zeptimeout ({entity_name})")
        except Exception as e:
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        if entity.related_edges:
            relationships = []
            for edge in entity.related_edges: # count
                fact = edge.get("fact", "")
        ...
        if entity.related_nodes:
            related_info = []
            for node in entity.related_nodes: # count
    ...
    def _generate_profile_with_llm(
        ...
        logger.warning(f"LLMfailed（{max_attempts}）: {last_error}, ")
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 7. failed，return
        logger.warning(f"JSONfailed，return")
        return {
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """get"""
        base_prompt = "。、simulation,max。returnJSON，。"
...
- """
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: graphID，Zepget
            parallel_count: count，default5
            realtime_output_path: writefilepath（，write）
            output_platform: platform ("reddit" "twitter")
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                logger.error(f" {entity.name} failed: {str(e)}")
                # createprofile
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    if error:
                        ...
                        logger.info(f"[{current}/{total}] success: {entity.name} ({entity_type})")
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        logger.info(f" {len(profiles)} Reddit Profile {file_path} (JSON，user_id)")
