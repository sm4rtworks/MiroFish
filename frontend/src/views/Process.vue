      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            ...
            <div class="action-buttons">
                <button class="action-btn" @click="refreshGraph" :disabled="graphLoading" title="graph">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">graphdata...</p>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">APIdescription</div>
                <div class="detail-content">
                  ，LLMcontent，simulation（type + type）
                </div>
              </div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">type ({{ projectData.ontology.entity_types?.length || 0 }})</div>
...
:key="entity.name"
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">type ({{ projectData.ontology.relation_types?.length || 0 }})</div>
...
:key="idx"
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 &&!ontologyProgress">
                <div class="waiting-hint">waiting...</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">graph</div>
        ...
        <div class="project-panel">
          ...
          <div class="project-details" v-if="projectData">
            <div class="project-item">
              <span class="item-label">projectname</span>
            ...
            <div class="project-item">
              <span class="item-label">simulation</span>
...
const currentPhase = ref(-1) // -1:, 0:, 1: graph, 2: 
const selectedItem = ref(null) // nodeedge
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 1) return 'graph'
...
// project - ontology/generate API
const handleNewProject = async () => {
  const pending = getPendingUpload()
  
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'file，return'
    loading.value = false
    return
  }
  ...
  try {
    ...
    ontologyProgress.value = { message: 'Uploaded files...' }
    ...
    error.value = 'projectinitializefailed: ' + (err.message || 'Error')
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || 'projectfailed'
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = 'projectfailed: ' + (err.message || 'Error')
  } finally {
    loading.value = false
  }
}
...
const updatePhaseByStatus = (status) => {
  switch (status) {
    ...
    case 'graph_completed':
      ...
      break
    case 'failed':
      error.value = projectData.value?.error || 'failed'
      break
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = 'graphtaskstart...'
      
      // task_id round
      const taskId = response.data.task_id
      
      // startgraphdataround（taskstatusround）
      startGraphPolling()
      
      // starttaskstatusround
      startPollingTask(taskId)
    } else {
      error.value = response.error || 'startgraphfailed'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'startgraphfailed: ' + (err.message || 'Error')
    buildProgress.value = null
  }
}
      ...
      if (task.status === 'completed') {
        ...
        if (projectResponse.success) {
          ...
          if (projectResponse.data.graph_id) {
            console.log('📊 graph:', projectResponse.data.graph_id)
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'graphfailed: ' + (task.error || 'Error')
        buildProgress.value = null
      }
    }
  } catch (err) {
