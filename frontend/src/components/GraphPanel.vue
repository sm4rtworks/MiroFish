...
const showEdgeLabels = ref(true) // defaultedge
const expandedSelfLoops = ref(new Set()) // 
const showSimulationFinishedHint = ref(false) // simulation
const wasSimulating = ref(false) // simulation
...
.attr('viewBox', `0 0 ${width} ${height}`)
  ...
  // edgedata，nodeedgecount
  const edgePairCount = {}
  const selfLoopEdges = {} // nodeedge
  const tempEdges = edgesData
.filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // statisticsnodeedgecount，edge
  tempEdges.forEach(e => {
...
...e,
  ...
  // nodeedge
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // node
    ...
    if (isSelfLoop) {
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // edge
        }
      })
      return
    }
...
.force('y', d3.forceY(height / 2).strength(0.04))
  ...
  const getLinkPath = (d) => {
    ...
    const offsetRatio = 0.25 + pairTotal * 0.05 // 25%，edge5%
